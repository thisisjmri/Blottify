...
class Libraries_Model extends Model{
    ...
    public function getBlotter(){
        $myStation=Session::get("station_id");
        $clickedStation=Session::get("cur_station");
        $myStationType=Session::get("station_type");


        if ($myStationType == "Main Station") {
            $dbh = new Database();
            $stmt = $dbh->prepare("SELECT * FROM `station` WHERE `id`= :clickedStation");
            $stmt->execute(compact('clickedStation'));
            ...
            if(empty($result)){
                ...
                $clickedStationType = $result['station_type'];

                if($myStationType == "Main Station" && $clickedStationType == "Sub Station"){
                    return $this->db->select("SELECT * FROM `blotter` WHERE `station_id` = '$clickedStation' AND `remarks` = 1");
                }
                else{
        ...
        else if ($myStationType == "Sub Station") {
            return $this->db->select("SELECT * FROM `blotter` WHERE `station_id` = '$myStation'");
        }
    ...
    public function setActive($data){
        ...
        $clickedStation=Session::get("cur_station");
        $myStationType=Session::get("station_type");
        $myStationType=Session::get("remarks");
        Session::init();
        ...
        if ($myStationType == "Main Station" && $clickedStationType == "Sub Station") {
            $dbh1 = new Database();
            $stmt1 = $dbh1->prepare("SELECT * FROM `blotter` WHERE `station_id`= :clickedStation AND `remarks` = 1");
            $stmt1->execute(compact('clickedStation'));
            ...
            $dbh2 = new Database();
            $stmt2 = $dbh2->prepare("SELECT * FROM `blotter` WHERE `station_id`= :myStation");
            $stmt2->execute(compact('myStation'));
    ...
    public function getFilter($data){
        ...
        if($station_type == "Main Station"){
          ...
          $sql = "SELECT * FROM `blotter` WHERE `station_id` = '$cur_station' AND `remarks` = '$remarks' ";
        }
        else{
          $cur_station = Session::get("station_id");
          $sql = "SELECT * FROM `blotter` WHERE `station_id` = '$cur_station' ";
        }
